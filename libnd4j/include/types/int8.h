...
namespace sd {
    ...
    _CUDA_HD float cpu_int82float(int8_t data) {
        return (float) ((int) data);
    }

    _CUDA_HD int8_t cpu_float2int8(float data) {
        int t = (int) data;
        if (t > 127) t = 127;
        if (t < -128) t = -128;
    ...
    template <class T>
    _CUDA_HD int8::int8(const T& rhs) {
        assign(rhs);
    }

    template <class T>
    _CUDA_HD int8& int8::operator=(const T& rhs) {
        assign(rhs); return *this;
    }


    _CUDA_HD int8::operator float() const {
        return cpu_int82float(data);
    }

    _CUDA_HD void int8::assign(double rhs) {
        assign((float)rhs);
    }

    _CUDA_HD void int8::assign(float rhs) {
