...
namespace sd {
    ...
    _CUDA_HD float cpu_uint82float(uint8_t data) {
        return static_cast<float>(static_cast<int>(data));
    }

    _CUDA_HD uint8_t cpu_float2uint8(float data) {
    ...
    _CUDA_HD uint8::uint8() { data = cpu_float2uint8(0.0f); }

    template <class T>
    _CUDA_HD uint8::uint8(const T& rhs) {
        assign(rhs);
    }

    template <class T>
    _CUDA_HD uint8& uint8::operator=(const T& rhs) { assign(rhs); return *this; }


    _CUDA_HD uint8::operator float() const {
        return cpu_uint82float(data);
    }

    _CUDA_HD void uint8::assign(double rhs) {
        assign(static_cast<float>(rhs));
    }

    _CUDA_HD void uint8::assign(float rhs) {
